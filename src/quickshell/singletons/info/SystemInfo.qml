...
Item {
    ...
    // Nerd Font distro glyphs (nf-linux-*), resolved from ID / ID_LIKE in /etc/os-release.
    readonly property string glyphArch: ""
    readonly property string glyphNix: ""
    readonly property string glyphTux: ""
    ...
    readonly property string distroIcon: {
        let id = ("" + distroId).toLowerCase();
        if (distroGlyphs[id]) return distroGlyphs[id];
        let like = ("" + distroIdLike).toLowerCase();
        if (id === "nixos" || like.indexOf("nixos") !== -1) return glyphNix;
        if (like.indexOf("arch") !== -1) return glyphArch;
        return glyphTux;
    }
        ...
        onLoaded: {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line.startsWith("PRETTY_NAME=")) {
                    pretty = line.split("=").slice(1).join("=").replace(/"/g, "").trim();
                } else if (line.startsWith("NAME=") && !root.osName) {
                    root.osName = line.split("=").slice(1).join("=").replace(/"/g, "").trim();
                } else if (line.startsWith("ID=")) {
                    root.distroId = line.split("=").slice(1).join("=").replace(/"/g, "").trim();
                } else if (line.startsWith("ID_LIKE=")) {
                    root.distroIdLike = line.split("=").slice(1).join("=").replace(/"/g, "").trim();
                }
            }
            if (pretty) root.osName = pretty;
        }
    }
