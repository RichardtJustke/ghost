...
Item {
                ...
                if (data) {
                    ...
                    if (data.timerTargetEpoch && data.timerTargetEpoch > 0) {
                        if (now >= data.timerTargetEpoch) {
                            ...
                            if (now - data.timerTargetEpoch < 60000) root.playAlarm();
                        } else {
                    ...
                    if (data.pomoTargetEpoch && data.pomoTargetEpoch > 0) {
                        if (now >= data.pomoTargetEpoch) {
                            ...
                            if (now - data.pomoTargetEpoch < 60000) root.playAlarm();
    ...
    // Audible signal played when a timer / pomodoro phase reaches zero.
    // Volume is controlled by the "timer alarm" slider in Guide -> Notifications
    // (independent from the global SFX volume so the alarm stays reliable).
    function playAlarm() {
        if (typeof Sounds === "undefined" || typeof Sounds.playSfx !== "function") return;
        let cfg = Config.getSetting("notifications", { "timerAlarmVolume": 100 });
        let vol = (cfg && cfg.timerAlarmVolume !== undefined) ? Number(cfg.timerAlarmVolume) : 100;
        vol = Math.max(0, Math.min(100, isNaN(vol) ? 100 : vol)) / 100;
        if (vol <= 0) return;
        Sounds.playSfx("quickactions/timer_complete.wav", vol, 0, true);
    }
