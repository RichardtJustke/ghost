...
PanelWindow {
    ...
    function loadApps() {
        ...
        // custom terminal commands (config-defined shortcuts, launched via the launcher's terminal)
        let cmdList = (typeof Config !== "undefined" && Config.rawSettings && Config.rawSettings.commands)
            ? Config.rawSettings.commands : [];
        for (let c = 0; c < cmdList.length; c++) {
            let cm = cmdList[c];
            if (!cm || !cm.exec) continue;
            let cScore = (usageRanks.launch && (usageRanks.launch[cm.name] || usageRanks.launch[cm.exec])) || 0;
            let custom = (cm.icon && ("" + cm.icon).length > 0 && ("" + cm.icon) !== "utilities-terminal") ? ("" + cm.icon) : "";
            arr.push({
                name: cm.name || cm.exec,
                description: (typeof I18n !== "undefined" ? I18n.t("applauncher.term_cmd") : "Terminal") + " · " + cm.exec,
                desktop_id: "term-cmd-" + (cm.id || c),
                icon: custom,
                fontIcon: custom.length ? "" : "󰆍",
                score: cScore,
                isCommand: false,
                command: "",
                isCalc: false,
    ...
    function executeFilter(query) {
        ...
        for (let i = 0; i < allApps.length; i++) {
            ...
            if (matches) {
                filtered.push({
                    ...
                    isTermCmd: app.isTermCmd || false,
                    termExec: app.termExec || ""
                });
            }
        }
        ...
        for (let i = 0; i < minCount; i++) {
            ...
            if (cur.name !== target.name
                ...
                || cur.isTermCmd !== target.isTermCmd
                || cur.termExec !== target.termExec) {
    ...
    function activateIndex(index) {
        ...
        if (item.isTermCmd) {
            let exec = item.termExec ? ("" + item.termExec).trim() : "";
            if (exec.length > 0) {
                let term = launcherWindow.terminalCommand ? launcherWindow.terminalCommand.trim() : "";
                let full = term ? (term + " " + exec) : exec;
                Quickshell.execDetached(["bash", "-lc", full]);
            }
            closeLauncher();
            return;
        }
