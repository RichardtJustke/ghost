...
in
{
  options.programs.serpantinum = {
    enable = mkEnableOption "the Ghost Quickshell desktop shell";

    package = mkOption {
      type = types.package;
      default = self.packages.${system}.default;
      defaultText = literalExpression "serpantinum.packages.<system>.default";
      description = "The Ghost package to use.";
    };
    ...
    settings = mkOption {
      ...
      '';
      description = ''
        Ghost configuration, layered on top of the package's
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.services.serpantinum = mkIf cfg.systemd.enable {
      Unit = {
        Description = "Ghost shell daemon";
